...
class DesktopService:
    ...
    def scan(self,library,extra=()):
        found=discovery.discover_games(Path(library),discovery.discover_nonsteam_roots(Path(library),[])) if library else []
        for path in extra:
            p=Path(path).resolve()
        ...
        for g in found:
            ...
            reason='Anti-cheat detected' if g.anti_cheat else 'No native DLSS-G detected' if not native else ''
            rows.append({'name':g.name,'game':str(root),'exe':Path(g.exe).relative_to(g.root).as_posix(),'source':g.type,'blocked':reason})
        return sorted(rows,key=lambda r:r['name'].casefold())

    def prepare(self,rows,mode,operation,adopt=False):
        import ui
        targets=app.normalize_targets([{**r,'mode':mode} for r in rows]);state=storage(self.config)
        ...
        for index,row in enumerate(targets,1):
            try:
                ...
                if plan['conflicts']:blocked.append({'name':row['name'],'reason':'\n'.join(plan['conflicts'])})
                elif plan['changes']:ready.append(plan)
                else:blocked.append({'name':row['name'],'reason':'Already up to date — no changes needed'})
            except (t.Refusal,OSError,ValueError) as ex:blocked.append({'name':row['name'],'reason':str(ex)})
        return {'kind':'batch','title':{'install':'Install / update','repair':'Repair','uninstall':'Remove owned files'}[operation],
                'plans':ready,'blocked':blocked,'rows':[{'name':Path(p['game']).name,'detail':f"{len(p['changes'])} file changes · {profiles.MODES[p['mode']]}"+(f" · Proton override: {Path(p['proxy']).stem}=n,b" if p.get('proxy') else '')} for p in ready]}
